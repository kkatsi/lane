<template>
  <Card v-if="!isEditing" class="py-2 gap-2 group px-2.5 block">
    ...
    <CardFooter class="bg-card border-t-0 p-0 py-2 text-muted-foreground">
      ...
      <Assignee v-if="!!assignee" class="ml-auto" :full-name="assignee?.name" size="sm" :color-id="assignee?.colorId"
        display-fullname />
    </CardFooter>
  </Card>
  <TaskComposer v-else :column-id="props.columnId" v-model:selected-label-ids="editLabelIds"
    v-model:selected-assignee-id="editAssigneeId" v-model:task-title="editTitle"
    v-model:task-description="editDescription" v-model:selected-due-date="editDueDate" @submit="onSubmit"
    @close="onClose" />
</template>
