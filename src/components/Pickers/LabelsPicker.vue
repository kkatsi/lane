<template>
  <Popover>
    <slot name="trigger" :count="selectedLabelIds.length" />
            ...
            <Popover v-model:open="isLabelColorOpen">
              <PopoverTrigger as-child>
                <button
                  ...
                  :style="{
                    backgroundColor: COLORS.find((c) => c.id === newLabelColor)?.background,
                  }"
                ></button>
              </PopoverTrigger>
              <PopoverContent class="p-0 w-auto" align="start" :side-offset="24" :align-offset="-8">
                <Item>
                  <label
                    :for="labelColor.id"
                    v-for="labelColor in COLORS"
                    class="w-5 h-5 rounded-full cursor-pointer"
                    :key="labelColor.id"
                    :style="{ backgroundColor: labelColor.background }"
                  >
                    <input
                      type="radio"
                      :id="labelColor.id"
