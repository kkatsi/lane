          ...
          <form.Field name="name">
            <template v-slot="{ field, state }">
              <Field :data-invalid="!state.meta.isValid">
                ...
                <Input :id="field.name" type="text" placeholder="e.g Mobile App" :name="field.name"
          ...
          <form.Field name="description">
            <template v-slot="{ field, state }">
              <Field :data-invalid="!state.meta.isValid">
                ...
                <Textarea :id="field.name" type="text" placeholder="What is the board for?" :name="field.name"
                  :model-value="state.value"
                  @update:model-value="(v) => field.handleChange(v as typeof field.state.value)" class="resize-none"
                  :aria-invalid="!state.meta.isValid" />
                <FieldError v-if="!state.meta.isValid" :errors="state.meta.errors" />
              </Field>
            </template>
          ...
          <form.Field name="colorId">
            <template v-slot="{ field, state }">
              <FieldSet :data-invalid="!state.meta.isValid">
                ...
                <RadioGroup :model-value="field.state.value"
                  @update:model-value="(v) => field.handleChange(v as typeof field.state.value)" class="flex gap-2"
                  :aria-invalid="!state.meta.isValid">
                  <FieldLabel v-for="COLOR in COLORS" :key="COLOR.id" :for="`form-radiogroup-${COLOR.id}`" :class="cn(
                    ...
                    <RadioGroupItem :id="`form-radiogroup-${COLOR.id}`" :value="COLOR.id"
                      :aria-invalid="!state.meta.isValid" class="sr-only" />
                  </FieldLabel>
                </RadioGroup>
                <FieldError v-if="!state.meta.isValid" :errors="state.meta.errors" />
              </FieldSet>
          ...
          <form.Field name="templateId">
            <template v-slot="{ field, state }">
              <FieldSet :data-invalid="!state.meta.isValid">
                ...
                <RadioGroup :model-value="field.state.value"
                  @update:model-value="(v) => field.handleChange(v as typeof field.state.value)"
                  :aria-invalid="!state.meta.isValid" class="gap-0">
                  <Field v-for="TEMPLATE in BOARD_TEMPLATES" :key="TEMPLATE.id" orientation="horizontal">
                    <FieldLabel :for="`form-radiogroup-${TEMPLATE.id}`" class="p-2 rounded-lg cursor-pointer">
                      <RadioGroupItem :id="`form-radiogroup-${TEMPLATE.id}`" :value="TEMPLATE.id"
                        :aria-invalid="!state.meta.isValid" />
